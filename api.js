...
const data = {
  ...
  2: [
    ...
    'blue',
    {
      ...
      6: ['People who think they know everything' + ' annoy those of us who do.'],
    },
  ],
  3: [
    'Ilene Dover',
  ...
  4: ['Lou D. Mouth', 'blue', {}],
  5: [
    'Vishwanath Subramanayam',
    'red',
    {
      42619: ['Too many cooks spoil the broth.', 'Many hands make light work.'],
  ...
  12: [
    ...
    'green',
    {
      146: ['The other quote is false.', 'The other quote is false.'],
  ...
  19: [
    ...
    {
      152: [
        'If you knock down the great wall of china,' +
          ' you will have to sweep up all the broken china',
        'No one has ever heard of the great wall of ceramic.',
      ],
      2: ['If a tree fell in the forest and no one was there to' + ' hear it, did it really fall?'],
    },
  ],
  20: [
    'Xhao Xin',
    'green',
    {
      ...
      30: [
        'If I told you once, I told you once.',
        'Oily to bed and oily to rise makes you greasy.',
      ],
      4: [
        'Little things come in small packages.',
        'Nothing worth having is easy.',
        "That sneeze is stronger than a bee's knees" + ' in a stiff breeze.',
      ],
      1926: [
        'Would would life be like if 12 were a prime number?',
...
app.get('/search', (req, res) => {
  ...
  if (term === undefined && color === undefined) {
    res.status(400).json({ error: 'at least one parameter of term or color is required' });
    return;
  }
  if (term !== undefined) {
  ...
  for (const [id, person] of Object.entries(data)) {
    ...
    if (term !== undefined) {
      if (
        person[0].toLowerCase().includes(term) &&
        (color === undefined || person[1].toLowerCase().includes(color))
      ) {
        match = true;
      }
    } else if (color !== undefined && person[1].toLowerCase().includes(color)) {
      match = true;
    }
    if (match) {
